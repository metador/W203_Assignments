...
hist(Data$NY.GDP.PCAP.CD,breaks = 100 )
```
...
>>We see that the model can explain 14% of the data.
...
```
>> we see that at the left side it is off but there are very fewe data points present in the left. Overall the line is almost centered at the center.
>> We can verify this by taking the mean or the sum of the residuals. Thus E(u_hat)~=0
...
```
>> We see there are 183 observations in the plot

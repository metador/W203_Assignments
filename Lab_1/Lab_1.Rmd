...
cor(log(CEO$mktval),log(CEO$profits),use = "complete.obs")
...
```{r}
ggplot(CEO,aes(x=age,y=salary,color = as.factor(grad)))+ geom_point()+ stat_smooth(method = "lm", se =F)
...
ggplot(mean_salary_age,aes(x=age,y=mean_salary,color = as.factor(1)))+ geom_point()  + stat_smooth(method = "lm", se =F)
cor(mean_salary_age)
```
...
ggplot(narrowed_mean_salary_age,aes(x=age,y=mean_salary,color = as.factor(1)))+ geom_point()  + stat_smooth(method = "lm", se =F)

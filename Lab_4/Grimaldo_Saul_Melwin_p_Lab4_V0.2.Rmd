...
Since we do know the data is from National Center for Health Statistics but we dont   how this data was sampled, there is the possibility that it might be from a state/region/county which has a large poplution of one ethnecity so might be some non-random factors. According to 2010 census data, 72% of people in the US are white. However, our data shows that both mom and dads are 88% white in the data. Unless our data came before a large change in the population happened, there are signs that the data might not be a random sample.
...
Below you can find the regressoin table summarizing the model results. 
...
stargazer(model1,model2,model3, type = "latex", report = "vc",
          ...
          title = "Linear model for predicting birthweight",
          keep.stat = c( "rsq","adj.rsq", "n","aic","ser"),
          omit.table.layout = "n")
```

Model1:

The estimated model1 is as follows
$maps_sum = 15.29 + 0.03683 * npvis$
...
We can say that it is a associative model and not a causal model. This is because the all the factors that might potientially effect the health of the new born baby is not captured in the variables of the model.